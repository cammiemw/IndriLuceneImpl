package org.lemurproject.inspectindex;
...
/**
 * A simple commandline utility for inspecting Lucene 8 indexes. Run it to see a
...
public class InspectIndex {
	...
	static String usage = "Usage:  java " + System.getProperty("sun.java.command") + " -index INDEX_PATH\n\n"
	 ...
	 */
	public static long getFieldLength(IndexReader reader, int docid, String fieldName) throws IOException {
	...
	public static void main(String[] args) throws IOException, Exception {
		...
		for (int i = 0; i < args.length; i++) {
			if (("-index".equals(args[i])) && ((i + 1) < args.length)) {
				reader = DirectoryReader.open(FSDirectory.open(Paths.get(args[i + 1])));
		...
		for (int i = 0; i < args.length; i++) {
			...
			if ("-index".equals(args[i])) {
				...
				System.out.println("Internal docid --> External docid: " + args[i + 1] + " --> " + d.get("externalId"));
				...
				System.out.println("\tkeywords:" + "\tnumdocs=" + reader.getDocCount("keywords") + "\tsumTotalTF="
						+ reader.getSumTotalTermFreq("keywords") + "\tavglen="
						+ reader.getSumTotalTermFreq("keywords") / (float) reader.getDocCount("keywords"));

				System.out.println("\ttitle:\t" + "\tnumdocs=" + reader.getDocCount("title") + "\tsumTotalTF="
						+ reader.getSumTotalTermFreq("title") + "\tavglen="
						+ reader.getSumTotalTermFreq("title") / (float) reader.getDocCount("title"));

				System.out.println("\tbody:\t" + "\tnumdocs=" + reader.getDocCount("body") + "\tsumTotalTF="
						+ reader.getSumTotalTermFreq("body") + "\tavglen="
						+ reader.getSumTotalTermFreq("body") / (float) reader.getDocCount("body"));

				System.out.println("\tinlink:\t" + "\tnumdocs=" + reader.getDocCount("inlink") + "\tsumTotalTF="
						+ reader.getSumTotalTermFreq("inlink") + "\tavglen="
	...
	static void listInternalDocid(IndexReader reader, String externalId) throws Exception {
		...
		LeafReaderContext leafContext = getLeafReaderContext(reader, externalId);
		...
		if (postings.nextDoc() != DocIdSetIterator.NO_MORE_DOCS) {
			...
			System.out.println("External docid --> Internal docid: " + externalId + " --> " + internalId);
			System.out.println("base: " + leafContext.docBase + ", " + "inc: " + postings.docID());
			return;
		}
		;
	...
	static void listMetadata(IndexReader reader, Integer docid) throws IOException {
		...
		while (fieldIterator.hasNext()) {
			...
			if (field.fieldType().indexOptions() == IndexOptions.DOCS) {
				...
				System.out.println("  Field: " + fieldName + "  length: " + fieldValues.length);
	...
	/**
	 * Displays the first n postings for a term in a field in an index (specified by
	 * reader). Set n to MAX_VALUE to display all postings.
	 * 
	 * @param reader An IndexReader
	 */
	static void listPostings(IndexReader reader, String termString, String field, Integer n) throws IOException {
	...
	/**
	 * Displays the term vectors for all of the fields in a document in an index
	 * (specified by reader).
	 * 
	 * @param reader An IndexReader
	 */
	static void listTermVectors(IndexReader reader, String docidString) throws IOException {
		...
		while (fieldIterator.hasNext()) {
			...
			System.out.println("    Stored length: " + getFieldLength(reader, docid, fieldName));
	...
	/**
	 * Displays the term vector for a field in a document in an index (specified by
	 * reader).
	 * 
	 * @param reader      An IndexReader
	 ...
	 * @return void
	 */
	static void listTermVectorField(IndexReader reader, String docidString, String field) throws IOException {
		...
		System.out.println("    Stored length: " + getFieldLength(reader, docid, field));
	...
	static void termVectorDisplay(Terms terms) throws IOException {
		...
		if ((terms == null) || (terms.size() == -1))
			...
			System.out.format("      %10s %-19s %s positions", " ", "term", "tf");
			System.out.println();

			while (ithTerm.next() != null) {
				System.out.format("      %10d %-20s %d ", ord, ithTerm.term().utf8ToString(), ithTerm.totalTermFreq());
				ord++;
